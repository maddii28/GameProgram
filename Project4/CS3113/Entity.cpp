...
void Entity::AIWander() {
    const float leftBound  = 200.0f;
    const float rightBound = 650.0f;

    if (mPosition.x <= leftBound)
    {
        moveRight();      
    }
    
    else if (mPosition.x >= rightBound)
    {
        moveLeft();      
    }
    else
...
void Entity::render()
{
    ...
    //modifying render logic for player specifically(flip textureArea to get left direction derived from right, 
    //also offset the render to match the colliderdimensions) - this is only for the main player

    Rectangle destinationArea;
    if (mEntityType == PLAYER || mEntityType == NPC){                    
        float frameWidth  = static_cast<float>(mTexture.width)  / mSpriteSheetDimensions.x;
        float frameHeight = static_cast<float>(mTexture.height) / mSpriteSheetDimensions.y;

        if (mDirection == LEFT)
        {
            textureArea.width = -textureArea.width;
            textureArea.x    += frameWidth;
        }
        if (mEntityType == PLAYER){
            destinationArea = {
            mPosition.x,
            mPosition.y - ((mScale.y - mColliderDimensions.y) / 2.0f),
